...
// Constructor 
Mrv::Mrv(int ndim,string pdfType, Array1D<int> rndInd, int order,string pctype, void* funcInfo)
{
...
int Mrv::Parametrize()
{
    ...
    for (int cur_rparam=0;cur_rparam<this->rDim_;cur_rparam++){
        ...
        // The PC PDT type dictates how the parameterization is done
...
Array2D<double> Mrv::propNISP(Array2D<double> (*forwardFcn)(Array2D<double>&, Array2D<double>&, void*), Array2D<double>& multiPCcf, Array2D<double>& x)
{
	Array2D<double> multiPCQuadSam=this->quadParam(multiPCcf);
	Array2D<double> funcQuad=forwardFcn(multiPCQuadSam,x, this->funcinfo_);
...
Array2D<double> Mrv::propMC(Array2D<double> (*forwardFcn)(Array2D<double>&, Array2D<double>&, void*), Array2D<double>& multiPCcf, Array2D<double>& x,int nsam)
{
	Array2D<double> multiPCSam=this->mcParam(multiPCcf, nsam);
	Array2D<double> funcSam=forwardFcn(multiPCSam,x, this->funcinfo_);
