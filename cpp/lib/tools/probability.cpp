...
// Returns the biased estimator of variance of an array of data
double get_var(const Array1D<double>& data)
{
  ...
  for(int i=0;i<ndata;i++) mean2 += data(i)*data(i);
  mean2=mean2/ndata;
               ...
               Array1D<double>& dens, int ncl, double sfac)
{
  ...
  for(int idim=0;idim<ndim;idim++){
    ...
    w(idim)=data_1d(maxIndex(data_1d))-data_1d(minIndex(data_1d));
  }
