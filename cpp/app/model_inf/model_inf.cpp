...
int usage(){
    ...
    printf("                 [-e<datanoise>] \n");
    printf("                 [-d<pdim>] [-o<order>] [-r<rndindfile>] [-s<pdftype>] [-c<pctype>] \n");
    ...
    printf(" -o <order>         : Parameter PDF order (default=%d) \n",ORDER);
...
{   
    ...
    int cc;
    while ((cc=getopt(argc,(char **)argv,"hf:l:w:k:i:a:b:j:x:y:t:d:o:r:s:c:e:m:g:q:p:u:n:z"))!=-1){
        switch (cc) {
            case 'h':
                usage();
                break;
    ...
    if (pflag){
        ...
        printf("Gamma parameter for MCMC               : %lg \n",mcmcgamma);
    }
    ...
    /// Parse modeltype options
    map<string, Array2D<double> (*)(Array2D<double>&, Array2D<double>&, void *) > func_dict = 
    {
        {"prop", Func_Prop}, {"prop_quad", Func_PropQuad},
        {"exp", Func_Exp}, {"exp_quad", Func_ExpQuad},
        {"const",Func_Const}, {"linear", Func_Linear}, {"bb", Func_BB},
        {"heat_transfer1", Func_HT1}, {"heat_transfer2", Func_HT2},
        {"frac_power", Func_FracPower},{"exp_sketch", Func_ExpSketch}, {"inp", Func_Inputs},
        {"pcl", Func_PCl}, {"pcx", Func_PCx}, {"pc", Func_PC},{"pcs", Func_PCs}
    };
    ...
    Array2D<double> (*forwardFunc)(Array2D<double>&, Array2D<double>&, void *);
