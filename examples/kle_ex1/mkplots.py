...
if rtype == "pltKLrecon1D":
    ...
    fs1 = 18
    xp=column(spls,0);
    npts=npy.array(xp).size
    savg=[npy.mean(spls[k][1::]) for k in range(npts)]
    fn=[]
    for n in range(Nkl):
        fn.append([0.0]*npts)
        for i in range(npts):
...
if rtype == "pltKLrecon2D":
    ...
    xi,nlines=readfile(fxi);
    savg=[npy.mean(samples[k][:]) for k in range(nx*ny)]
    fn=[]
    for n in range(Nkl):
        fn.append([0.0]*nx*ny)
    ...
    for n in range(0,Nkl,step):
        Zex=npy.array(column(samples,ids)).reshape(nx,ny)
        Z=npy.array(fn[n]).reshape(nx,ny)
        fig = plt.figure(figsize=(4,4))
...
if rtype == "xidata1D":
    ...
    Nxi  = 4
    for i in range(Nxi):
        x=npy.linspace(npy.array(column(din,i)).min(),npy.array(column(din,i)).max(),100)
        kerns=stats.kde.gaussian_kde(column(din,i))
        pleg.append(plt.plot(x,kerns(x),linewidth=lw1))
    #pleg.append(plt.plot(x,npy.exp(-x**2/(2*sigma*sigma))/(npy.sqrt(2*npy.pi)*sigma),linewidth=lw))
...
if rtype == "samples2Du":
    ...
    x = npy.array(column(xy,0))
    y = npy.array(column(xy,1))
...
if rtype == "anlKLevec2Du":
    ...
    din,nlines=readfile(fname);
    x = npy.array(column(din,0))
    y = npy.array(column(din,1))
    # Create the Triangulation; no triangles so Delaunay triangulation created.
    triGrid = tri.Triangulation(x,y)
    # Mask off unwanted triangles.
    xyc,nl=readfile('data/cali.dat')
